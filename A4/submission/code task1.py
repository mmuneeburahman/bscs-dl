...
Original file is located at
    https://colab.research.google.com/drive/1dVusJ4djg3CJpdRl1PEVV4p8EJYRzagE
...
def loadDataset(path):
    ...
    for i in range(10):
        for filename in os.listdir(path+"/train/"+classes[i]):
            train.append(path+"/train/"+classes[i]+"/"+filename)
    for i in range(10):
        for filename in os.listdir(path+"/test/"+classes[i]):
            test.append(path+"/test/"+classes[i]+"/"+filename)
...
def train(d, g, epochs, loss, loader, d_optimizer, g_optimizer, fixed_noise,  save_images=True):
    ...
    for epoch in range(epochs):
        ...
        for real_images in loader:
            ...
            g.zero_grad()
            labels = torch.ones(batch_size, 1, 1, 1, device=device) #label is one
            output_dg = d(fake_images)
            g_loss = loss(output_dg.float(), labels.float())
            g_loss.backward()
            g_optimizer.step()
...
def main(train_net, save_net, save_images, plot_train_results):
    ...
    train_dataset = Real(train_data,  transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
    test_dataset = Real(test_data, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
    ...
    if train_net:
      D_LOSSES, G_LOSSES = train(d,g, EPOCHS, LOSS_FN, train_loader, D_OPTIM, G_OPTIM, fixed_noise, save_images)
    else:
      load_model(g, PATH_TO_LOAD_MODEL+"t1_g.pt")
      load_model(d, PATH_TO_LOAD_MODEL+"t1_d.pt")
    if save_net:
      save_model(g, PATH_TO_SAVE_MODEL+"t1_g.pt")
      save_model(d, PATH_TO_SAVE_MODEL+"t1_d.pt")
