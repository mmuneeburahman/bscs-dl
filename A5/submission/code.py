...
def prepocess_input_sequence(
    ...
    sequence "03", you should break it down to a list of digits,
...
def scaled_dot_product_two_loop_single(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.matmul(query, key.T) #5x4 (4x5) = 5x5
    out/=torch.sqrt(torch.tensor(query.size(1)))
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.matmul(query, torch.transpose(key, 1, 2)) #5x4 (4x5) = 5x5
    sqrt_M = torch.sqrt(torch.tensor(query.size(1)))
    out/= sqrt_M
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        # Replace "pass" statement with your code
        pass
        # torch.where(mask, -1*torch.exp(9))
    # Replace "pass" statement with your code
    y = torch.matmul(query, torch.transpose(key, 1, 2)) #5x4 (4x5) = 5x5
    sqrt_M = torch.sqrt(torch.tensor(query.size(1)))
    y/= sqrt_M
    y = torch.softmax(y, dim=2)
    y = torch.matmul(y, value)
    weights_softmax = torch.softmax(y, dim=2)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.xavier_uniform_(self.q.weight, gain=1)
        nn.init.xavier_uniform_(self.k.weight, gain=1)
        nn.init.xavier_uniform_(self.v.weight, gain=1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        key_ = self.k(key)
        value_ = self.v(value)
        y, weights_softmax = scaled_dot_product_no_loop_batch(query_, key_, value_, mask)
        self.weights_softmax = weights_softmax
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        print(f'num_heads: {num_heads}, dim_in: {dim_in}, dim_out: {dim_out}')
        self.ml = nn.ModuleList(SelfAttention(dim_in, dim_out, dim_out) for i in range(num_heads))

        self.l = nn.Linear(dim_out*num_heads, dim_in)
        
        nn.init.xavier_uniform_(self.l.weight, gain=1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for self_attention in self.ml:
            out = self_attention(query, key, value, mask)
        ...
        # print("self.l.weights.shape: ", self.l.weight.size())
        y = self.l(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        We compute the  mean with shape (N, K) and standard deviation with shape (N, K) 
        ...
        # Dont forget to encapsulate these scale and shift initializations with  #
        # nn.Parameter                                                           #
        ##########################################################################
        # Replace "pass" statement with your code
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = self.scale*(x-x.mean(dim=-1, keepdim=True))/(x.std(dim=-1, keepdim=True)+self.epsilon)+self.shift
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    N = seq.shape[0]
    K = seq.shape[1]
    mask = torch.triu(torch.ones(N, K, K, dtype=bool),1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out2 = self.dropout(y)
        # print(f'dec.shape: {dec_inp.shape}, out2.shape: {out2.shape}, enc_inp.shape: {enc_inp.shape}')
        out3 = self.attention_cross(dec_inp, enc_inp, enc_inp )
        y = self.norm2(out2+out3)
        y = self.dropout(y)
        out5 = self.feed_forward(y)
        y = self.norm3(y+out5)
        y = self.dropout(y)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.tensor([[n/K] for n in range(K)]).repeat(1, M).unsqueeze(0)
    # print("y.shape ", y.shape)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for p in range(K):
        ...
        for i in range(M):
            if i%2==0:
                row.append(sin(p/(10000**(2*i/M))))
            else:
                row.append(cos(p/(10000**(2*i/M))))
        mat.append(row)
    y = torch.tensor(mat).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # print(a_emb_inp.shape)
        mask = get_subsequent_mask(torch.ones(a_emb_inp.shape[0], a_emb_inp.shape[1]))
